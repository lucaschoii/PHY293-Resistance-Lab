...
def R_squared_test(m, b, x, y):
    

    y_fit = x*m + b
    diff = (y - y_fit)**2
    sum_diff = sum(diff)
    s_yx_squared = sum_diff/(len(y) - 2)

    y_mean = sum(y)/len(y)
    sum_diff_y = sum((y - y_mean)**2)


    r_squared = 1 - s_yx_squared * (len(y) - 2)/sum_diff_y
    return r_squared

def R_squared_test2(m, b, x, y):
    # Calculate the predicted y values using the linear equation y_fit = mx + b
    y_fit = x * m + b
...
def chi_squared_test(m, b, x, y, yerr):
    residules = y - (m*x + b)
    numerator = residules**2
    denominator = yerr**2
...
def plot_fit(my_func, xdata, ydata, xerror=None, yerror=None, init_guess=None, font_size=14, title="",
